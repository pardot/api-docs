...
| `batchCreate` | `/api/prospect /version/3 /do/batchCreate? prospects=<data>...` | `user_key, api_key, prospects` | Creates new prospects using the provided `<data>` in either XML or JSON. See [Endpoints for Batch Processing](#endpoints-for-batch-processing) Note: Will assign the prospects to optional parameter `<user_id>` if passed in. 
...
| `batchUpsert` | `/api/prospect /version/3 /do/batchUpsert? prospects=<data>...` | `user_key, api_key, prospects` | Updates prospects using the provided `<data>` in either XML or JSON. See [Endpoints for Batch Processing](#endpoints-for-batch-processing). Note: Will assign the prospects to optional parameter `<user_id>` if passed in.
