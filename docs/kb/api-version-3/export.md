...
* **type**: (Optional) Selects visitor activities of the specified types. If this argument is not specified, then all of the visitor acitivies belonging to any type is returned. See a list of available [Visitor Activity Types](../object-field-references.md#visitor-activity-types) in [Visitor Activity](../object-field-references.md#visitor-activity) in [Object Field References](../object-field-references.md).
* **prospectOnly**: (Optional) Selects only those visitor acitivites assosiated to a prospect. When this field is set to `false`, all visitor acitivites with and without a prospect are returned. The values can be `true` or `false`.
