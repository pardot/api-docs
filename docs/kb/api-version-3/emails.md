...
| **Operation** | **URL Format**                             | **Required Parameters** | **Description**  |
| ------------- | ------------------------------------------ | ----------------------- | -----------------|
| `stats` | `/api/email/version/3/do/stats/id/<list email id>?...` | `user_key, api_key, email` | Returns the statistical data for the list email specified by `<id>`. `<id>` is the Pardot ID of the target target email.
