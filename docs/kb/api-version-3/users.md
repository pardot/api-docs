...
| ------------- | ------------------------------------------ | ----------------------- | -----------------|
| `query` | `/api/user/version/3/do/query?...` | `user_key, api_key` | Returns the users matching the specified criteria parameters. See the [Using Users](#using-users) section for a complete description of the user [XML Response Format](#xml-response-format). Also see [User](../object-field-references.md#user) in [Object Field References](../object-field-references.md). |
