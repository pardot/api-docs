...
| **Operation** | **URL Format**                             | **Required Parameters** | **Description**  |
| ------------- | ------------------------------------------ | ----------------------- | -----------------|
| `read` | `/api/customField/version/3/do/read/id/<id>?...` | `user_key, api_key, id` | Returns the data for the custom field specified by `<id>`. `<id>` is the Pardot ID of the target custom field. |
| `update` | `/api/customField/version/3/do/update/id/<id>?...` | `user_key, api_key, id` | Updates the provided data for the custom field specified by `<id>`.  `<id>` is the Pardot ID of the custom field. Refer to [Custom Field](../object-field-references#custom-field) in [Object Field References](../object-field-references) for more details. Returns the updated version of the custom field. |
| `create` | `/api/customField/version/3/do/create?...` | `user_key, api_key` | Creates a new custom field using the specified data. |
| `delete` | `/api/customField/version/3/do/delete/id/<id>?...` | `user_key, api_key, id` | Deletes the custom field specified by `<id>`. Returns HTTP 204 No Content on success. **_Note:_** Custom Fields may only be deleted using HTTP methods POST or DELETE. |
