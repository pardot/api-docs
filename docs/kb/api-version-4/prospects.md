...
| `upsert`      | `/api/prospect /version/4 /do/upsert/ {id/<id> AND/OR fid/<fid> AND/OR email/<email>}?...`   | `user_key, api_key, (id, fid, email)` | [Upserting a Prospect](#upserting-prospects). Note: Will assign the prospect to optional parameter `<user_id>` if passed in. 
| `batchUpsert` | `/api/prospect /version/4 /do/batchUpsert? prospects=<data>...` | `user_key, api_key, prospects` | Updates prospects using the provided `<data>` in either XML or JSON. See [Endpoints for Batch Processing](#endpoints-for-batch-processing). Note: Will assign the prospects to optional parameter `<user_id>` if passed in. 
